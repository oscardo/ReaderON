...
import { DoMakeItem, DO_MAKE_DATA } from '../constants/doMake';
import { DoMakeGuide } from './DoMakeGuide';
import {
  getAllFlags,
  setWordFlag,
  resetWordFlag,
  getDoMakeRepetitions,
  incrementDoMakeRepetition,
  clearAllDoMakePersistence,
  clearFlagsInRange
} from '../services/dbService';

interface DoMakeSectionProps {
  onBack: () => void;
...
const DoMakeSection: React.FC<DoMakeSectionProps> = ({ onBack, speakText }) => {
  const [activeTab, setActiveTab] = useState<'STUDY' | 'PLAY' | 'GUIDE'>('STUDY');
  const [activeGame, setActiveGame] = useState<GameType | null>(null);
  const [selectedIndex, setSelectedIndex] = useState<number | null>(null);
  const [activeFilter, setActiveFilter] = useState<'ALL' | 'DO' | 'MAKE'>('ALL');
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        for (const s of DO_MAKE_DATA) {
          const rep = await getDoMakeRepetitions(s.id);
          if (rep > 0) repMap[s.id] = rep;
        }
        setRepetitions(repMap);
      } catch (err) {
        console.error("Error loading DoMake stats:", err);
      }
    };
    loadStats();
  }, []);
  ...
  const filteredItems = useMemo(() => {
    let base = DO_MAKE_DATA;
    if (activeFilter !== 'ALL') {
      base = base.filter(s => s.type === activeFilter);
    }
    if (showFlaggedOnly) {
      base = base.filter(s => (flags[s.id + 18000] || 0) > 0);
    }
    return base;
  }, [showFlaggedOnly, flags, activeFilter]);

  const progress = useMemo(() => {
    const total = DO_MAKE_DATA.length;
    const flagged = DO_MAKE_DATA.filter(v => (flags[v.id + 18000] || 0) > 0).length;
  ...
  const handleFlagPress = async (e: React.MouseEvent) => {
    ...
    const wordId = selectedItem.id + 18000;
  ...
  const handlePlay = async (word: string, id: number) => {
    speakText(word);
    await incrementDoMakeRepetition(id);
    const newCount = await getDoMakeRepetitions(id);
    setRepetitions(prev => ({ ...prev, [id]: newCount }));
  };

  const questions = useMemo(() => {
    return [...DO_MAKE_DATA].sort(() => Math.random() - 0.5).slice(0, 10).map(s => {
      const options = [s.spanish];
      while (options.length < 4) {
        const randomS = DO_MAKE_DATA[Math.floor(Math.random() * DO_MAKE_DATA.length)];
          ...
          <>
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2 uppercase">
                      DO <span className="text-indigo-600">vs</span> MAKE
                    </h2>
                    <p className="text-[8px] font-black text-indigo-500 uppercase tracking-widest">Execution vs Creation</p>
                  </div>
                </div>
                  ...
                  <button
                    onClick={async () => {
                      if (window.confirm('¿Deseas reiniciar todas las banderas y repeticiones de esta sección?')) {
                        await clearAllDoMakePersistence();
                        await clearFlagsInRange(18000, 18999);
                        setFlags({});
                        setRepetitions({});
              ...
              <div className="flex gap-2 overflow-x-auto no-scrollbar pb-3 px-1">
                {(['ALL', 'DO', 'MAKE'] as const).map(filter => (
                  <button
                    key={filter}
                    onClick={() => setActiveFilter(filter)}
                    className={`px-4 py-1.5 rounded-full text-[8px] font-black whitespace-nowrap uppercase tracking-widest transition-all border ${activeFilter === filter
              ...
              {activeTab === 'STUDY' && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between px-2">
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Phrases Dictionary</h3>
                    ...
                    {filteredItems.map((s, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-4 text-left">
                          <div className={`w-8 h-8 rounded-xl ${s.type === 'DO' ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'} flex items-center justify-center text-[10px] font-black shadow-inner`}>
                            {s.type === 'DO' ? 'D' : 'M'}
                          </div>
                          <div>
                        ...
                        <div className="flex items-center gap-3">
                          {(flags[s.id + 18000] || 0) > 0 && (
                            <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[8px] font-black ${(flags[s.id + 18000] || 0) >= 3 ? 'bg-rose-500 text-white' : 'bg-amber-400 text-white'}`}>
                              <Flag size={8} fill="currentColor" />
                              <span>{(flags[s.id + 18000] || 0)}</span>
                            </div>
                          )}
                  ...
                  {!gameStarted ? (
                    <div className="grid grid-cols-1 gap-4 pt-4">
                      ...
                      >
                        ...
                        <h3 className="text-lg font-black text-slate-800 mb-2">Concept Matcher</h3>
                        <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-6">Link actions to correct verbs</p>
                      ...
                      <motion.button
                        ...
                        className="p-8 bg-gradient-to-br from-slate-800 to-slate-950 rounded-[2.5rem] shadow-xl flex flex-col items-center text-center group text-white"
                      >
                        <div className="w-16 h-16 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                          <Sparkles size={32} />
                        </div>
                        <h3 className="text-lg font-black mb-2">Master Challenge</h3>
                        <p className="text-white/70 text-[10px] font-bold uppercase tracking-widest mb-6">Expert level DO/MAKE activity</p>
                        <div className="px-6 py-2 bg-white text-slate-900 rounded-xl font-black text-[9px] uppercase tracking-widest flex items-center gap-2">
                          BEGIN NOW <Zap size={12} fill="currentColor" />
                        </div>
                      </motion.button>
                    </div>
                  ) : (
                    <div className="flex-1 flex flex-col">
                      {gameFinished ? (
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex-1 flex flex-col items-center justify-center p-8 bg-white rounded-[3rem] border border-slate-100 shadow-xl">
                          ...
                          <h3 className="text-2xl font-black text-slate-800 mb-2">Mission Accomplished!</h3>
                        ...
                        <div className="space-y-6">
                          <div className="flex items-center justify-between px-2">
                            <div className="flex flex-col gap-1">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Goal {currentQuestion + 1}/10</span>
                              </div>
                          ...
                          <AnimatePresence mode="wait">
                            ...
                            >
                              <div className={`absolute top-0 left-0 w-full h-1 ${questions[currentQuestion].type === 'DO' ? 'bg-indigo-500' : 'bg-amber-500'}`} />
                              <h4 className="text-2xl font-black text-slate-800 tracking-tight mb-1 italic">"{questions[currentQuestion].word}"</h4>
                              <p className="text-slate-400 font-black text-[8px] uppercase tracking-widest opacity-60">Selecciona la traducción correcta</p>
                            </motion.div>
                          </AnimatePresence>
                            ...
                            {questions[currentQuestion].options.map((opt, idx) => {
                              ...
                              return (
                                <motion.button
                                  ...
                                  className={`p-5 rounded-2xl font-bold text-[11px] text-left transition-all border-2 flex items-center justify-between ${showRes && isCorrect
                          ...
                          <div className="flex justify-center pt-4">
                            <button onClick={() => setGameStarted(false)} className="text-[9px] font-black text-slate-300 uppercase tracking-widest hover:text-rose-500 transition-colors">Quit Mission</button>
                          </div>
                        </div>
                      )}
                    </div>
          ...
          >
            ...
            <div className="flex-1 flex items-center justify-center p-6">
              <div className="w-full max-w-sm bg-white rounded-[3rem] p-8 shadow-2xl relative overflow-hidden flex flex-col">
                <div className={`absolute top-0 left-0 w-full h-1.5 ${selectedItem.type === 'DO' ? 'bg-indigo-500' : 'bg-amber-500'}`} />

                <div className="flex items-center justify-between mb-8">
                  <div className={`px-3 py-1 rounded-full ${selectedItem.type === 'DO' ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'} font-black text-[7px] uppercase tracking-widest border border-slate-100`}>
                    {selectedItem.type} CATEGORY
                  </div>
                  <div className="flex gap-2">
                    <button onClick={handleFlagPress} className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${getFlagColor(flags[selectedItem.id + 18000] || 0)}`}>
                      <Flag size={14} fill={(flags[selectedItem.id + 18000] || 0) > 0 ? "currentColor" : "none"} />
                    </button>
                ...
                <div className="text-center space-y-4 mb-8">
                  <h3 className="text-4xl font-black text-slate-900 tracking-tight leading-tight uppercase">{selectedItem.word}</h3>
                  <div className="inline-block px-4 py-1 bg-slate-100 rounded-full text-slate-500 font-black text-[9px] tracking-[0.2em] uppercase">
                ...
                <div className="space-y-4 flex-1">
                  <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">En Español</p>
                    <p className="text-xl font-black text-slate-800">{selectedItem.spanish}</p>
                  </div>
                  <div className={`p-5 ${selectedItem.type === 'DO' ? 'bg-indigo-50/50' : 'bg-amber-50/50'} rounded-2xl border border-slate-100`}>
                    <p className={`text-[8px] font-black ${selectedItem.type === 'DO' ? 'text-indigo-500' : 'text-amber-600'} uppercase tracking-widest mb-1`}>Contexto / Regla</p>
                    <p className="text-xs font-bold text-slate-600 leading-relaxed italic">{selectedItem.context}</p>
                  </div>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-slate-50 flex gap-3">
                  ...
                  <button onClick={() => setSelectedIndex(prev => prev! < filteredItems.length - 1 ? prev! + 1 : prev)} className="w-12 h-12 rounded-2xl bg-slate-100 text-slate-400 flex items-center justify-center active:scale-90 transition-all">
