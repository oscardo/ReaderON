...
import { getAllFlags, setWordFlag, resetWordFlag, getPhrasalVerbUpOutRepetitions, incrementPhrasalVerbUpOutRepetition, clearAllPhrasalVerbUpOutPersistence, clearFlagsInRange } from '../services/dbService';
...
const PhrasalVerbsUpOutSection: React.FC<PhrasalVerbsUpOutSectionProps> = ({ onBack, speakText }) => {
  ...
  const filteredPhrasalVerbsUpOut = useMemo(() => {
    let base = PHRASAL_VERBS_UP_OUT;
    if (showFlaggedOnly) {
      base = base.filter(s => (flags[s.id + 7000] || 0) > 0);
    }
    return base;
  }, [showFlaggedOnly, flags]);

  const totalPages = Math.ceil(filteredPhrasalVerbsUpOut.length / pageSize);
  const listPhrasalVerbsUpOut = useMemo(() => {
    return filteredPhrasalVerbsUpOut.slice(currentPage * pageSize, (currentPage + 1) * pageSize);
  }, [filteredPhrasalVerbsUpOut, currentPage]);

  const selectedPhrasalVerbUpOut = selectedIndex !== null ? filteredPhrasalVerbsUpOut[selectedIndex] : null;

  const handleFlagPress = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!selectedPhrasalVerbUpOut) return;
    const wordId = selectedPhrasalVerbUpOut.id + 7000;
  ...
  const goToNext = (e?: React.MouseEvent) => {
    ...
    if (nextIdx < filteredPhrasalVerbsUpOut.length) {
      setSelectedIndex(nextIdx);
      handlePlay(filteredPhrasalVerbsUpOut[nextIdx].word, filteredPhrasalVerbsUpOut[nextIdx].id);
    } else {
  ...
  const goToPrevious = (e?: React.MouseEvent) => {
    ...
    if (prevIdx >= 0) {
      ...
      handlePlay(filteredPhrasalVerbsUpOut[prevIdx].word, filteredPhrasalVerbsUpOut[prevIdx].id);
    } else {
      setSelectedIndex(null);
    }
  };
              ...
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <h2 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2 uppercase">
                      Phrasal Verbs <span className="text-rose-600">UP & OUT</span>
                    </h2>
                ...
                <button
                  onClick={async () => {
                    if (window.confirm('¿Deseas reiniciar todas las banderas y repeticiones de esta sección?')) {
                      await clearAllPhrasalVerbUpOutPersistence();
                      await clearFlagsInRange(7000, 7999);
                      setFlags({});
                      setRepetitions({});
              ...
              <div className="flex items-center gap-2 mt-2">
                 <div className="flex-1 overflow-x-auto pb-2 scrollbar-hide flex gap-2">
                   {totalPages > 1 && !showFlaggedOnly && Array.from({ length: totalPages }).map((_, i) => (
                     ...
                     >
                       {i * pageSize + 1} - {Math.min((i + 1) * pageSize, PHRASAL_VERBS_UP_OUT.length)}
                     </button>
                   ))}
                 </div>
                 <button 
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2 pb-24">
        {activeTab === 'STUDY' ? (
          <div className="space-y-2">
            {listPhrasalVerbsUpOut.map((s, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    {(flags[s.id + 7000] || 0) > 0 && (
                      <div className={`w-2 h-2 rounded-full shadow-sm ${(flags[s.id + 7000] || 0) >= 3 ? 'bg-rose-500' : 'bg-amber-500'}`} />
                    )}
                      ...
                      <div className="w-full space-y-3 mb-10 max-w-sm">
                         {score === 10 ? (
                           <div className="p-5 bg-gradient-to-r from-rose-500 to-pink-600 rounded-3xl text-white font-black text-sm shadow-lg">
                             🏆 PERFECT! You've mastered "UP & OUT"!
                           </div>
                         ) : score >= 7 ? (
          ...
          <PhrasalVerbsUpOutGuide />
        ) : null}
      </div>

      {/* Detail Overlay */}
      <AnimatePresence>
        {selectedIndex !== null && selectedPhrasalVerbUpOut && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            <div className="flex-1 flex items-center justify-center p-6">
              <div className="flex items-center justify-center w-full max-w-3xl gap-4 h-full">
                ...
                <motion.div
                  key={selectedPhrasalVerbUpOut.id}
                  ...
                  <div className="text-center flex-1 flex flex-col pt-4">
                    ...
                    <div className="space-y-4 mb-10 overflow-y-auto max-h-[30vh] px-2 scrollbar-hide">
                      <div className="p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100 shadow-inner text-left">
                        ...
                        <p className="text-slate-900 text-3xl font-black tracking-tighter leading-tight mb-2">{selectedPhrasalVerbUpOut.spanish}</p>
                        <div className="inline-block px-3 py-1 rounded-lg bg-rose-100/50 text-rose-700 font-black text-[9px] uppercase tracking-widest">
                           {selectedPhrasalVerbUpOut.category}
                        </div>
                      </div>
                      ...
                      <div className="p-6 bg-indigo-50/50 rounded-[2.5rem] border border-indigo-100/20 relative text-left">
                        ...
                        <p className="text-slate-700 text-sm font-bold leading-relaxed">
                          "{selectedPhrasalVerbUpOut.meaning}"
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mt-auto pt-6 border-t border-slate-50">
                      <button
                        onClick={() => handlePlay(selectedPhrasalVerbUpOut.word, selectedPhrasalVerbUpOut.id)}
                      ...
                      <div className="flex flex-col items-center gap-2 shrink-0">
                        <button
                          ...
                          className={`w-14 h-14 rounded-[1.8rem] border-2 flex items-center justify-center shadow-lg active:scale-90 transition-all ${
                            getFlagColor(flags[selectedPhrasalVerbUpOut.id + 7000] || 0)
                          }`}
                        >
                          <Flag size={24} fill={(flags[selectedPhrasalVerbUpOut.id + 7000] || 0) > 0 ? 'currentColor' : 'none'} strokeWidth={3} />
                        </button>
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest opacity-50">Flag Lv {(flags[selectedPhrasalVerbUpOut.id + 7000] || 0)}</span>
                      </div>
