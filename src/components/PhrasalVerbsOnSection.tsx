...
import { getAllFlags, setWordFlag, resetWordFlag, getPhrasalVerbOnRepetitions, incrementPhrasalVerbOnRepetition, clearAllPhrasalVerbOnPersistence, clearFlagsInRange } from '../services/dbService';
...
const PhrasalVerbsOnSection: React.FC<PhrasalVerbsOnSectionProps> = ({ onBack, speakText }) => {
  ...
  const filteredPhrasalVerbsOn = useMemo(() => {
    let base = PHRASAL_VERBS_ON;
    if (showFlaggedOnly) {
      base = base.filter(s => (flags[s.id + 9000] || 0) > 0);
    }
    return base;
  }, [showFlaggedOnly, flags]);

  const totalPages = Math.ceil(filteredPhrasalVerbsOn.length / pageSize);
  const listPhrasalVerbsOn = useMemo(() => {
    return filteredPhrasalVerbsOn.slice(currentPage * pageSize, (currentPage + 1) * pageSize);
  }, [filteredPhrasalVerbsOn, currentPage]);

  const selectedPhrasalVerbOn = selectedIndex !== null ? filteredPhrasalVerbsOn[selectedIndex] : null;

  const handleFlagPress = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!selectedPhrasalVerbOn) return;
    const wordId = selectedPhrasalVerbOn.id + 9000;
  ...
  const goToNext = (e?: React.MouseEvent) => {
    ...
    if (nextIdx < filteredPhrasalVerbsOn.length) {
      setSelectedIndex(nextIdx);
      handlePlay(filteredPhrasalVerbsOn[nextIdx].word, filteredPhrasalVerbsOn[nextIdx].id);
    } else {
  ...
  const goToPrevious = (e?: React.MouseEvent) => {
    ...
    if (prevIdx >= 0) {
      ...
      handlePlay(filteredPhrasalVerbsOn[prevIdx].word, filteredPhrasalVerbsOn[prevIdx].id);
    } else {
      setSelectedIndex(null);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative overflow-hidden">
              ...
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <h2 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2 uppercase">
                      Phrasal Verbs <span className="text-emerald-600">ON</span>
                    </h2>
                    <p className="text-[8px] font-black text-emerald-500 uppercase tracking-widest">Interactive Learning</p>
                 </div>
              </div>
                ...
                <button
                  onClick={async () => {
                    if (window.confirm('¿Deseas reiniciar todas las banderas y repeticiones de esta sección?')) {
                      await clearAllPhrasalVerbOnPersistence();
                      await clearFlagsInRange(9000, 9999);
                      setFlags({});
                      setRepetitions({});
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl shadow-inner">
                    {(['GUIDE', 'STUDY', 'PLAY'] as const).map(tab => (
                        <button 
                            ...
                            className={`px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400'}`}
                        >
                            {tab}
                        </button>
                    ))}
              ...
              <div className="flex items-center gap-2 mt-2">
                 <div className="flex-1 overflow-x-auto pb-2 scrollbar-hide flex gap-2">
                   {totalPages > 1 && !showFlaggedOnly && Array.from({ length: totalPages }).map((_, i) => (
                     ...
                     >
                       {i * pageSize + 1} - {Math.min((i + 1) * pageSize, PHRASAL_VERBS_ON.length)}
                     </button>
                   ))}
                 </div>
                 <button 
            ...
            {listPhrasalVerbsOn.map((s, idx) => {
              ...
              return (
                <motion.button
                  ...
                  className="w-full flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 shadow-sm hover:border-emerald-200 transition-all group"
                >
                  <div className="flex items-center gap-4 text-left">
                    <div className="w-8 h-8 rounded-xl bg-emerald-50 flex items-center justify-center text-[10px] font-black text-emerald-500 shadow-inner">
                      {absoluteIdx + 1}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    {(flags[s.id + 9000] || 0) > 0 && (
                      <div className={`w-2 h-2 rounded-full shadow-sm ${(flags[s.id + 9000] || 0) >= 3 ? 'bg-rose-500' : 'bg-amber-500'}`} />
                    )}
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-white rounded-[3rem] border border-slate-100 shadow-xl min-h-[450px] relative overflow-hidden">
                ...
                {gameFinished ? (
                   <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center relative z-10">
                      <div className="w-24 h-24 rounded-[2rem] bg-emerald-50 flex items-center justify-center text-emerald-500 mb-6 shadow-xl border border-emerald-100/50">
                        <Trophy size={48} strokeWidth={2.5} />
                      </div>
                      <h3 className="text-3xl font-black text-slate-800 mb-2 tracking-tight">Challenge Results</h3>
                      <p className="text-slate-500 font-black mb-8 uppercase tracking-[0.2em] text-xs">Score: <span className="text-emerald-600">{score}/10</span> • Max Streak: <span className="text-amber-500">{maxStreak}</span></p>
                      ...
                      <div className="w-full space-y-3 mb-10 max-w-sm">
                         {score === 10 ? (
                           <div className="p-5 bg-gradient-to-r from-emerald-500 to-teal-600 rounded-3xl text-white font-black text-sm shadow-lg">
                             🏆 PERFECT! You've mastered "ON"!
                           </div>
                         ) : score >= 7 ? (
                           <div className="p-5 bg-emerald-50 rounded-3xl border-2 border-emerald-100 text-emerald-700 font-black text-sm">
                  ...
                  <>
                    <div className="w-20 h-20 rounded-[2rem] bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-white mb-8 shadow-xl shadow-emerald-100">
                    ...
                    <button 
                      onClick={startNewGame}
                      className="w-full max-w-xs py-5 bg-emerald-600 text-white rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest shadow-xl shadow-emerald-100 active:scale-95 flex items-center justify-center gap-3"
                    >
              ...
              <div className="flex-1 flex flex-col pt-2">
                <div className="flex items-center justify-between mb-6 px-2">
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="h-2 w-40 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                      <motion.div animate={{ width: `${((currentQuestion + 1) / 10) * 100}%` }} className="h-full bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.6)]" />
                    </div>
                  </div>
                ...
                <div className="flex-1 flex flex-col justify-center gap-4">
                  ...
                  >
                    ...
                    <p className="text-emerald-500 font-black text-[10px] uppercase tracking-[0.3em] opacity-50">Select the meaning</p>
                  </motion.div>
                        ...
                        <motion.button
                          ...
                          className={`p-6 rounded-[1.8rem] font-black text-[13px] transition-all border-2 text-left flex items-center justify-between group relative overflow-hidden ${
                            isSelected 
                              ...
                              : 'bg-white border-slate-100 text-slate-700 active:bg-slate-50 hover:border-emerald-300 shadow-sm'
                          }`}
                        >
                          ...
                          {!isSelected && <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 group-hover:bg-emerald-50 group-hover:text-emerald-500 transition-all shrink-0">
      ...
      {/* Detail Overlay */}
      <AnimatePresence>
        {selectedIndex !== null && selectedPhrasalVerbOn && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
                ...
                >
                  ...
                  <div className="text-center flex-1 flex flex-col pt-4">
                    <div className="flex flex-col items-center gap-4 mb-8">
                      <div className="px-3 py-1 rounded-full bg-emerald-50 text-emerald-600 font-black text-[8px] uppercase tracking-[0.2em] shadow-inner border border-emerald-100/50">
                        ON MODULE • UNIT {selectedPhrasalVerbOn.id}
                      </div>
                      <h3 className="text-5xl font-black text-slate-900 tracking-tighter leading-none">
                        {selectedPhrasalVerbOn.word}
                      </h3>
                      <div className="bg-slate-50 px-4 py-2 rounded-2xl border border-slate-100">
                        <span className="text-indigo-500 font-black text-xl italic tracking-tight">
                            {selectedPhrasalVerbOn.pronunciation}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 mb-10 overflow-y-auto max-h-[30vh] px-2 scrollbar-hide">
                      <div className="p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100 shadow-inner text-left">
                        ...
                        <p className="text-slate-900 text-3xl font-black tracking-tighter leading-tight mb-2">{selectedPhrasalVerbOn.spanish}</p>
                        <div className="inline-block px-3 py-1 rounded-lg bg-emerald-100/50 text-emerald-700 font-black text-[9px] uppercase tracking-widest">
                           {selectedPhrasalVerbOn.category}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-3 mt-auto pt-6 border-t border-slate-50">
                      <button
                        onClick={() => handlePlay(selectedPhrasalVerbOn.word, selectedPhrasalVerbOn.id)}
                      ...
                      <div className="flex flex-col items-center gap-2 shrink-0">
                        <button
                          ...
                          className={`w-14 h-14 rounded-[1.8rem] border-2 flex items-center justify-center shadow-lg active:scale-90 transition-all ${
                            getFlagColor(flags[selectedPhrasalVerbOn.id + 9000] || 0)
                          }`}
                        >
                          <Flag size={24} fill={(flags[selectedPhrasalVerbOn.id + 9000] || 0) > 0 ? 'currentColor' : 'none'} strokeWidth={3} />
                        </button>
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest opacity-50">Flag Lv {(flags[selectedPhrasalVerbOn.id + 9000] || 0)}</span>
                      </div>
