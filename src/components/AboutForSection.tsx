...
import { PhrasalVerbAboutFor, PHRASAL_VERBS_ABOUT_FOR } from '../constants/aboutFor';
import { AboutForGuide } from './AboutForGuide';
import { 
  getAllFlags, 
  setWordFlag, 
  resetWordFlag, 
  getAboutForRepetitions, 
  incrementAboutForRepetition, 
  clearAllAboutForPersistence, 
  clearFlagsInRange 
} from '../services/dbService';

interface AboutForSectionProps {
  onBack: () => void;
...
const AboutForSection: React.FC<AboutForSectionProps> = ({ onBack, speakText }) => {
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        for (const s of PHRASAL_VERBS_ABOUT_FOR) {
          const rep = await getAboutForRepetitions(s.id);
          if (rep > 0) repMap[s.id] = rep;
        }
        setRepetitions(repMap);
      } catch (err) {
        console.error("Error loading AboutFor stats:", err);
      }
    };
    loadStats();
  }, []);
  ...
  const filteredVerbs = useMemo(() => {
    let base = PHRASAL_VERBS_ABOUT_FOR;
    if (activeCategory !== 'ALL') {
      base = base.filter(s => s.category === activeCategory);
    }
    if (showFlaggedOnly) {
      base = base.filter(s => (flags[s.id + 20000] || 0) > 0);
    }
    return base;
  }, [showFlaggedOnly, flags, activeCategory]);

  const categories = ['ALL', 'ABOUT', 'FOR'];

  const progress = useMemo(() => {
    const total = PHRASAL_VERBS_ABOUT_FOR.length;
    const flagged = PHRASAL_VERBS_ABOUT_FOR.filter(v => (flags[v.id + 20000] || 0) > 0).length;
  ...
  const handleFlagPress = async (e: React.MouseEvent) => {
    ...
    const wordId = selectedVerb.id + 20000;
  ...
  const handlePlay = async (word: string, id: number) => {
    ...
    speakText(cleanWord);
    await incrementAboutForRepetition(id);
    const newCount = await getAboutForRepetitions(id);
    setRepetitions(prev => ({ ...prev, [id]: newCount }));
  };

  const questions = useMemo(() => {
    return [...PHRASAL_VERBS_ABOUT_FOR].sort(() => Math.random() - 0.5).slice(0, 10).map(s => {
      const options = [s.spanish];
      while (options.length < 4) {
        const randomS = PHRASAL_VERBS_ABOUT_FOR[Math.floor(Math.random() * PHRASAL_VERBS_ABOUT_FOR.length)];
            ...
            >
                ...
                <div className="flex items-center gap-3">
                   ...
                   <div>
                      <h2 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2 uppercase">
                        ABOUT <span className="text-indigo-600">&</span> FOR
                      </h2>
                      <p className="text-[8px] font-black text-indigo-500 uppercase tracking-widest">Intention Module</p>
                   </div>
                </div>
                  ...
                  <button
                    onClick={async () => {
                      if (window.confirm('¿Deseas reiniciar todas las banderas y repeticiones de esta sección?')) {
                        await clearAllAboutForPersistence();
                        await clearFlagsInRange(20000, 20999);
                        setFlags({});
                        setRepetitions({});
              ...
              <div className="flex-1 mb-4">
                   <div className="flex justify-between items-end mb-1 px-1">
                      <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Mastery Progress</span>
              ...
              {activeTab === 'STUDY' && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between px-2">
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Phrasal Verbs List</h3>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {filteredVerbs.map((s) => {
                      const absoluteIdx = PHRASAL_VERBS_ABOUT_FOR.findIndex(v => v.id === s.id);
                      return (
                        <motion.button
                          key={s.id}
                          whileTap={{ scale: 0.98 }}
                        ...
                        >
                          <div className="flex items-center gap-4 text-left">
                            <div className={`w-8 h-8 rounded-xl ${s.category === 'ABOUT' ? 'bg-indigo-50 text-indigo-500' : 'bg-rose-50 text-rose-600'} flex items-center justify-center text-[10px] font-black shadow-inner`}>
                              {absoluteIdx + 1}
                            </div>
                            <div>
                          ...
                          <div className="flex items-center gap-3">
                            {(flags[s.id + 20000] || 0) > 0 && (
                              <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[8px] font-black ${(flags[s.id + 20000] || 0) >= 3 ? 'bg-rose-500 text-white' : 'bg-amber-400 text-white'}`}>
                                <Flag size={8} fill="currentColor" />
                                <span>{(flags[s.id + 20000] || 0)}</span>
                              </div>
                            )}
                  ...
                  {!gameStarted ? (
                    <div className="grid grid-cols-1 gap-4 pt-4">
                      <motion.button 
                        whileHover={{ y: -4 }}
                        whileTap={{ scale: 0.98 }}
                        onClick={() => { setActiveGame('MATCH'); startNewGame(); }}
                      ...
                      >
                        ...
                        <h3 className="text-lg font-black text-slate-800 mb-2">About & For Match</h3>
                    ...
                    <div className="flex-1 flex flex-col">
                      {gameFinished ? (
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex-1 flex flex-col items-center justify-center p-8 bg-white rounded-[3rem] border border-slate-100 shadow-xl">
                          ...
                          <h3 className="text-2xl font-black text-slate-800 mb-2">Great Job!</h3>
                        ...
                        <div className="space-y-6">
                           ...
                           <AnimatePresence mode="wait">
                              ...
                              >
                                 ...
                                 <p className="text-indigo-500 font-black text-[8px] uppercase tracking-widest opacity-60">Selecciona la traducción correcta</p>
                              </motion.div>
                           </AnimatePresence>
          ...
          >
            ...
            <div className="flex-1 flex items-center justify-center p-6">
               <div className="w-full max-w-sm bg-white rounded-[3rem] p-8 shadow-2xl relative overflow-hidden flex flex-col">
                  <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-indigo-400 to-rose-600" />
                  ...
                  <div className="flex items-center justify-between mb-8">
                     ...
                     <div className="flex gap-2">
                        <button onClick={handleFlagPress} className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${getFlagColor(flags[selectedVerb.id + 20000] || 0)}`}>
                           <Flag size={14} fill={(flags[selectedVerb.id + 20000] || 0) > 0 ? "currentColor" : "none"} />
                        </button>
                  ...
                  <div className="space-y-4 flex-1">
                     ...
                     <div className="p-5 bg-indigo-50/50 rounded-2xl border border-indigo-100/50">
                        <p className="text-[8px] font-black text-indigo-500 uppercase tracking-widest mb-1">Concept</p>
...
export default AboutForSection;
