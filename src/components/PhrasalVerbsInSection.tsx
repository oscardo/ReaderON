...
import { PhrasalVerbIn, PHRASAL_VERBS_IN } from '../constants/phrasalVerbsIn';
import { PhrasalVerbsInGuide } from './PhrasalVerbsInGuide';
import { getAllFlags, setWordFlag, resetWordFlag, getPhrasalVerbInRepetitions, incrementPhrasalVerbInRepetition, clearAllPhrasalVerbInPersistence, clearFlagsInRange } from '../services/dbService';

interface PhrasalVerbsInSectionProps {
  onBack: () => void;
  speakText: (text: string) => void;
}

const PhrasalVerbsInSection: React.FC<PhrasalVerbsInSectionProps> = ({ onBack, speakText }) => {
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        for (const s of PHRASAL_VERBS_IN) {
          const rep = await getPhrasalVerbInRepetitions(s.id);
          if (rep > 0) repMap[s.id] = rep;
        }
        setRepetitions(repMap);
      } catch (err) {
        console.error("Error loading PhrasalVerbIn stats:", err);
      }
    };
    loadStats();
  }, []);

  const filteredPhrasalVerbsIn = useMemo(() => {
    let base = PHRASAL_VERBS_IN;
    if (showFlaggedOnly) {
      base = base.filter(s => (flags[s.id + 10000] || 0) > 0);
    }
    return base;
  }, [showFlaggedOnly, flags]);

  const totalPages = Math.ceil(filteredPhrasalVerbsIn.length / pageSize);
  const listPhrasalVerbsIn = useMemo(() => {
    return filteredPhrasalVerbsIn.slice(currentPage * pageSize, (currentPage + 1) * pageSize);
  }, [filteredPhrasalVerbsIn, currentPage]);

  const selectedPhrasalVerbIn = selectedIndex !== null ? filteredPhrasalVerbsIn[selectedIndex] : null;

  const handleFlagPress = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!selectedPhrasalVerbIn) return;
    const wordId = selectedPhrasalVerbIn.id + 10000;
  ...
  const handlePlay = async (word: string, id: number) => {
    ...
    speakText(cleanWord);
    await incrementPhrasalVerbInRepetition(id);
    const newCount = await getPhrasalVerbInRepetitions(id);
  ...
  const goToNext = (e?: React.MouseEvent) => {
    ...
    if (nextIdx < filteredPhrasalVerbsIn.length) {
      setSelectedIndex(nextIdx);
      handlePlay(filteredPhrasalVerbsIn[nextIdx].word, filteredPhrasalVerbsIn[nextIdx].id);
    } else {
  ...
  const goToPrevious = (e?: React.MouseEvent) => {
    ...
    if (prevIdx >= 0) {
      ...
      handlePlay(filteredPhrasalVerbsIn[prevIdx].word, filteredPhrasalVerbsIn[prevIdx].id);
    } else {
      setSelectedIndex(null);
    }
  };
  ...
  const questions = useMemo(() => {
    return [...PHRASAL_VERBS_IN].sort(() => Math.random() - 0.5).slice(0, 10).map(s => {
      const options = [s.meaning];
      while (options.length < 4) {
        const randomS = PHRASAL_VERBS_IN[Math.floor(Math.random() * PHRASAL_VERBS_IN.length)];
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative overflow-hidden">
              ...
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <h2 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2 uppercase">
                      Phrasal Verbs <span className="text-teal-600">IN</span>
                    </h2>
                    <p className="text-[8px] font-black text-teal-500 uppercase tracking-widest">Interactive Learning</p>
                 </div>
              </div>
                ...
                <button
                  onClick={async () => {
                    if (window.confirm('¿Deseas reiniciar todas las banderas y repeticiones de esta sección?')) {
                      await clearAllPhrasalVerbInPersistence();
                      await clearFlagsInRange(10000, 10999);
                      setFlags({});
                      setRepetitions({});
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl shadow-inner">
                    {(['GUIDE', 'STUDY', 'PLAY'] as const).map(tab => (
                        <button 
                            ...
                            className={`px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-400'}`}
                        >
                            {tab}
                        </button>
                    ))}
              ...
              <div className="flex items-center gap-2 mt-2">
                 <div className="flex-1 overflow-x-auto pb-2 scrollbar-hide flex gap-2">
                   {totalPages > 1 && !showFlaggedOnly && Array.from({ length: totalPages }).map((_, i) => (
                     ...
                     >
                       {i * pageSize + 1} - {Math.min((i + 1) * pageSize, PHRASAL_VERBS_IN.length)}
                     </button>
                   ))}
                 </div>
                 <button 
            ...
            {listPhrasalVerbsIn.map((s, idx) => {
              ...
              return (
                <motion.button
                  ...
                  className="w-full flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 shadow-sm hover:border-teal-200 transition-all group"
                >
                  <div className="flex items-center gap-4 text-left">
                    <div className="w-8 h-8 rounded-xl bg-teal-50 flex items-center justify-center text-[10px] font-black text-teal-500 shadow-inner">
                      {absoluteIdx + 1}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    {(flags[s.id + 10000] || 0) > 0 && (
                      <div className={`w-2 h-2 rounded-full shadow-sm ${(flags[s.id + 10000] || 0) >= 3 ? 'bg-rose-500' : 'bg-amber-500'}`} />
                    )}
          ...
          <div className="h-full flex flex-col">
            {!gameStarted ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-white rounded-[3rem] border border-slate-100 shadow-xl min-h-[450px] relative overflow-hidden">
                <div className="absolute -top-24 -right-24 w-64 h-64 bg-teal-50 rounded-full blur-3xl opacity-50" />
                ...
                {gameFinished ? (
                   <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center relative z-10">
                      <div className="w-24 h-24 rounded-[2rem] bg-teal-50 flex items-center justify-center text-teal-500 mb-6 shadow-xl border border-teal-100/50">
                        <Trophy size={48} strokeWidth={2.5} />
                      </div>
                      <h3 className="text-3xl font-black text-slate-800 mb-2 tracking-tight">Challenge Results</h3>
                      <p className="text-slate-500 font-black mb-8 uppercase tracking-[0.2em] text-xs">Score: <span className="text-teal-600">{score}/10</span> • Max Streak: <span className="text-amber-500">{maxStreak}</span></p>
                      
                      <div className="w-full space-y-3 mb-10 max-w-sm">
                         {score === 10 ? (
                           <div className="p-5 bg-gradient-to-r from-emerald-500 to-teal-600 rounded-3xl text-white font-black text-sm shadow-lg">
                             🏆 PERFECT! You've mastered "IN"!
                           </div>
                         ) : score >= 7 ? (
                           <div className="p-5 bg-teal-50 rounded-3xl border-2 border-teal-100 text-teal-700 font-black text-sm">
                  ...
                  <>
                    <div className="w-20 h-20 rounded-[2rem] bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center text-white mb-8 shadow-xl shadow-teal-100">
                    ...
                    <button 
                      onClick={startNewGame}
                      className="w-full max-w-xs py-5 bg-teal-600 text-white rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest shadow-xl shadow-teal-100 active:scale-95 flex items-center justify-center gap-3"
                    >
              ...
              <div className="flex-1 flex flex-col pt-2">
                <div className="flex items-center justify-between mb-6 px-2">
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="h-2 w-40 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                      <motion.div animate={{ width: `${((currentQuestion + 1) / 10) * 100}%` }} className="h-full bg-teal-500 shadow-[0_0_12px_rgba(20,184,166,0.6)]" />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-xl font-black text-teal-600 leading-none">{score}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col justify-center gap-4">
                  ...
                  >
                    ...
                    <p className="text-teal-500 font-black text-[10px] uppercase tracking-[0.3em] opacity-50">Select the meaning</p>
                  </motion.div>
                        ...
                        <motion.button
                          ...
                          className={`p-6 rounded-[1.8rem] font-black text-[13px] transition-all border-2 text-left flex items-center justify-between group relative overflow-hidden ${
                            isSelected 
                              ...
                              : 'bg-white border-slate-100 text-slate-700 active:bg-slate-50 hover:border-teal-300 shadow-sm'
                          }`}
                        >
                          ...
                          {!isSelected && <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 group-hover:bg-teal-50 group-hover:text-teal-500 transition-all shrink-0">
      ...
      {/* Detail Overlay */}
      <AnimatePresence>
        {selectedIndex !== null && selectedPhrasalVerbIn && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
                ...
                >
                  ...
                  <div className="text-center flex-1 flex flex-col pt-4">
                    <div className="flex flex-col items-center gap-4 mb-8">
                      <div className="px-3 py-1 rounded-full bg-teal-50 text-teal-600 font-black text-[8px] uppercase tracking-[0.2em] shadow-inner border border-teal-100/50">
                        IN MODULE • UNIT {selectedPhrasalVerbIn.id}
                      </div>
                      <h3 className="text-5xl font-black text-slate-900 tracking-tighter leading-none">
                        {selectedPhrasalVerbIn.word}
                      </h3>
                      <div className="bg-slate-50 px-4 py-2 rounded-2xl border border-slate-100">
                        <span className="text-indigo-500 font-black text-xl italic tracking-tight">
                            {selectedPhrasalVerbIn.pronunciation}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 mb-10 overflow-y-auto max-h-[30vh] px-2 scrollbar-hide">
                      <div className="p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100 shadow-inner text-left">
                        ...
                        <p className="text-slate-900 text-3xl font-black tracking-tighter leading-tight mb-2">{selectedPhrasalVerbIn.spanish}</p>
                        <div className="inline-block px-3 py-1 rounded-lg bg-teal-100/50 text-teal-700 font-black text-[9px] uppercase tracking-widest">
                           {selectedPhrasalVerbIn.category}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-3 mt-auto pt-6 border-t border-slate-50">
                      <button
                        onClick={() => handlePlay(selectedPhrasalVerbIn.word, selectedPhrasalVerbIn.id)}
                      ...
                      <div className="flex flex-col items-center gap-2 shrink-0">
                        <button
                          ...
                          className={`w-14 h-14 rounded-[1.8rem] border-2 flex items-center justify-center shadow-lg active:scale-90 transition-all ${
                            getFlagColor(flags[selectedPhrasalVerbIn.id + 10000] || 0)
                          }`}
                        >
                          <Flag size={24} fill={(flags[selectedPhrasalVerbIn.id + 10000] || 0) > 0 ? 'currentColor' : 'none'} strokeWidth={3} />
                        </button>
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest opacity-50">Flag Lv {(flags[selectedPhrasalVerbIn.id + 10000] || 0)}</span>
                      </div>
...
export default PhrasalVerbsInSection;
