...
import { 
  ...
  setWordFlag, 
  resetWordFlag, 
  getPhrasalVerbBackRepetitions, 
  incrementPhrasalVerbBackRepetition, 
  clearAllPhrasalVerbBackPersistence, 
  clearFlagsInRange 
...
const PhrasalVerbsBackSection: React.FC<PhrasalVerbsBackSectionProps> = ({ onBack, speakText }) => {
  ...
  const categories = useMemo(() => {
    const cats = Array.from(new Set(PHRASAL_VERBS_BACK.map(v => v.category)));
    return ['ALL', ...cats];
  }, []);

  const progress = useMemo(() => {
    const total = PHRASAL_VERBS_BACK.length;
    const flagged = PHRASAL_VERBS_BACK.filter(v => (flags[v.id + 11000] || 0) > 0).length;
        ...
        {selectedIndex === null && (
          <>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2 pb-24">
                ...
                <div className="space-y-6">
                  ...
                  {activeCategory === 'ALL' ? (
                    categories.filter(c => c !== 'ALL').map(cat => {
                      const verbsInCat = PHRASAL_VERBS_BACK.filter(v => v.category === cat);
                      ...
                      return (
                        <div key={cat} className="space-y-3">
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {verbsInCat.map((s) => {
                              const absoluteIdx = PHRASAL_VERBS_BACK.findIndex(v => v.id === s.id);
                              return (
                                <motion.button
                                  key={s.id}
                                  whileTap={{ scale: 0.98 }}
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-3">
                                    {(flags[s.id + 11000] || 0) > 0 && (
                                      <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[8px] font-black ${(flags[s.id + 11000] || 0) >= 3 ? 'bg-rose-500 text-white' : 'bg-amber-400 text-white'}`}>
                                        <Flag size={8} fill="currentColor" />
                                        <span>{(flags[s.id + 11000] || 0)}</span>
                                      </div>
                                    )}
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {filteredPhrasalVerbsBack.map((s) => {
                        const absoluteIdx = PHRASAL_VERBS_BACK.findIndex(v => v.id === s.id);
                        return (
                          <motion.button
                            key={s.id}
                        ...
                        <div className="space-y-6">
                           ...
                           <AnimatePresence mode="wait">
                              ...
                              >
                                 ...
                                 <p className="text-indigo-500 font-black text-[8px] uppercase tracking-widest opacity-60">What does it mean?</p>
                              </motion.div>
                           </AnimatePresence>
              ...
              {activeTab === 'GUIDE' && <PhrasalVerbsBackGuide />}
            </div>
          </>
        )}
      </AnimatePresence>
            ...
            <div className="flex-1 flex items-center justify-center p-6">
               <div className="w-full max-w-sm bg-white rounded-[3rem] p-8 shadow-2xl relative overflow-hidden flex flex-col">
                  ...
                  <div className="flex items-center justify-between mb-8">
                     ...
                     <div className="flex gap-2">
                        <button onClick={handleFlagPress} className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${getFlagColor(flags[selectedPhrasalVerbBack.id + 11000] || 0)}`}>
                           <Flag size={14} fill={(flags[selectedPhrasalVerbBack.id + 11000] || 0) > 0 ? "currentColor" : "none"} />
                        </button>
                  ...
                  <div className="text-center space-y-4 mb-8">
                     <h3 className="text-4xl font-black text-slate-900 tracking-tight">{selectedPhrasalVerbBack.word}</h3>
                  ...
                  <div className="space-y-4 flex-1">
                     <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
                        ...
                        <p className="text-xl font-black text-slate-800">{selectedPhrasalVerbBack.spanish}</p>
                     </div>
                     <div className="p-5 bg-indigo-50/50 rounded-2xl border border-indigo-100/50">
                        <p className="text-[8px] font-black text-indigo-500 uppercase tracking-widest mb-1">Conceptual Meaning</p>
                        <p className="text-xs font-bold text-slate-600 leading-relaxed italic">"{selectedPhrasalVerbBack.meaning}"</p>
                     </div>
                  </div>
