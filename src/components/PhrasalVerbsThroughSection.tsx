...
import { PhrasalVerbThrough, PHRASAL_VERBS_THROUGH } from '../constants/phrasalVerbsThrough';
import { PhrasalVerbsThroughGuide } from './PhrasalVerbsThroughGuide';
import { getAllFlags, setWordFlag, resetWordFlag, getPhrasalVerbThroughRepetitions, incrementPhrasalVerbThroughRepetition, clearAllPhrasalVerbThroughPersistence, clearFlagsInRange } from '../services/dbService';

interface PhrasalVerbsThroughSectionProps {
  onBack: () => void;
  speakText: (text: string) => void;
}

const PhrasalVerbsThroughSection: React.FC<PhrasalVerbsThroughSectionProps> = ({ onBack, speakText }) => {
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        for (const s of PHRASAL_VERBS_THROUGH) {
          const rep = await getPhrasalVerbThroughRepetitions(s.id);
  ...
  const filteredPhrasalVerbsThrough = useMemo(() => {
    let base = PHRASAL_VERBS_THROUGH;
    if (showFlaggedOnly) {
      base = base.filter(s => (flags[s.id + 12000] || 0) > 0);
    }
    return base;
  }, [showFlaggedOnly, flags]);

  const totalPages = Math.ceil(filteredPhrasalVerbsThrough.length / pageSize);
  const listPhrasalVerbsThrough = useMemo(() => {
    return filteredPhrasalVerbsThrough.slice(currentPage * pageSize, (currentPage + 1) * pageSize);
  }, [filteredPhrasalVerbsThrough, currentPage]);

  const selectedPhrasalVerbThrough = selectedIndex !== null ? filteredPhrasalVerbsThrough[selectedIndex] : null;

  const handleFlagPress = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!selectedPhrasalVerbThrough) return;
    const wordId = selectedPhrasalVerbThrough.id + 12000;
  ...
  const handlePlay = async (word: string, id: number) => {
    ...
    speakText(cleanWord);
    await incrementPhrasalVerbThroughRepetition(id);
    const newCount = await getPhrasalVerbThroughRepetitions(id);
  ...
  const goToNext = (e?: React.MouseEvent) => {
    ...
    if (nextIdx < filteredPhrasalVerbsThrough.length) {
      setSelectedIndex(nextIdx);
      handlePlay(filteredPhrasalVerbsThrough[nextIdx].word, filteredPhrasalVerbsThrough[nextIdx].id);
    } else {
  ...
  const goToPrevious = (e?: React.MouseEvent) => {
    ...
    if (prevIdx >= 0) {
      ...
      handlePlay(filteredPhrasalVerbsThrough[prevIdx].word, filteredPhrasalVerbsThrough[prevIdx].id);
    } else {
      setSelectedIndex(null);
    }
  };
  ...
  const questions = useMemo(() => {
    return [...PHRASAL_VERBS_THROUGH].sort(() => Math.random() - 0.5).slice(0, 10).map(s => {
      const options = [s.meaning];
      while (options.length < 4) {
        const randomS = PHRASAL_VERBS_THROUGH[Math.floor(Math.random() * PHRASAL_VERBS_THROUGH.length)];
              ...
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <h2 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2 uppercase">
                      Phrasal Verbs <span className="text-rose-600">THROUGH</span>
                    </h2>
                ...
                <button
                  onClick={async () => {
                    if (window.confirm('¿Deseas reiniciar todas las banderas y repeticiones de esta sección?')) {
                      await clearAllPhrasalVerbThroughPersistence();
                      await clearFlagsInRange(12000, 12999);
                      setFlags({});
                      setRepetitions({});
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl shadow-inner">
                    {(['GUIDE', 'STUDY', 'PLAY'] as const).map(tab => (
                        <button 
                            ...
                            className={`px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-400'}`}
                        >
                            {tab}
                        </button>
                    ))}
              ...
              <div className="flex items-center gap-2 mt-2">
                 <div className="flex-1 overflow-x-auto pb-2 scrollbar-hide flex gap-2">
                   {totalPages > 1 && !showFlaggedOnly && Array.from({ length: totalPages }).map((_, i) => (
                     ...
                     >
                       {i * pageSize + 1} - {Math.min((i + 1) * pageSize, PHRASAL_VERBS_THROUGH.length)}
                     </button>
                   ))}
                 </div>
                 <button 
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2 pb-24">
        {activeTab === 'STUDY' ? (
          <div className="space-y-2">
            {listPhrasalVerbsThrough.map((s, idx) => {
              ...
              return (
                <motion.button
                  ...
                  className="w-full flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 shadow-sm hover:border-rose-200 transition-all group"
                >
                  <div className="flex items-center gap-4 text-left">
                    <div className="w-8 h-8 rounded-xl bg-rose-50 flex items-center justify-center text-[10px] font-black text-rose-500 shadow-inner">
                      {absoluteIdx + 1}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    {(flags[s.id + 12000] || 0) > 0 && (
                      <div className={`w-2 h-2 rounded-full shadow-sm ${(flags[s.id + 12000] || 0) >= 3 ? 'bg-rose-500' : 'bg-amber-500'}`} />
                    )}
          ...
          <div className="h-full flex flex-col">
            {!gameStarted ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-white rounded-[3rem] border border-slate-100 shadow-xl min-h-[450px] relative overflow-hidden">
                <div className="absolute -top-24 -right-24 w-64 h-64 bg-rose-50 rounded-full blur-3xl opacity-50" />
                
                {gameFinished ? (
                   <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center relative z-10">
                      <div className="w-24 h-24 rounded-[2rem] bg-rose-50 flex items-center justify-center text-rose-500 mb-6 shadow-xl border border-rose-100/50">
                        <Trophy size={48} strokeWidth={2.5} />
                      </div>
                      <h3 className="text-3xl font-black text-slate-800 mb-2 tracking-tight">Challenge Results</h3>
                      <p className="text-slate-500 font-black mb-8 uppercase tracking-[0.2em] text-xs">Score: <span className="text-rose-600">{score}/10</span> • Max Streak: <span className="text-amber-500">{maxStreak}</span></p>
                      
                      <div className="w-full space-y-3 mb-10 max-w-sm">
                         {score === 10 ? (
                           <div className="p-5 bg-gradient-to-r from-rose-500 to-pink-600 rounded-3xl text-white font-black text-sm shadow-lg">
                             🏆 PERFECT! You've mastered "THROUGH"!
                           </div>
                         ) : score >= 7 ? (
                           <div className="p-5 bg-rose-50 rounded-3xl border-2 border-rose-100 text-rose-700 font-black text-sm">
                  ...
                  <>
                    <div className="w-20 h-20 rounded-[2rem] bg-gradient-to-br from-rose-400 to-rose-600 flex items-center justify-center text-white mb-8 shadow-xl shadow-rose-100">
                    ...
                    <button 
                      onClick={startNewGame}
                      className="w-full max-w-xs py-5 bg-rose-600 text-white rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest shadow-xl shadow-rose-100 active:scale-95 flex items-center justify-center gap-3"
                    >
              ...
              <div className="flex-1 flex flex-col pt-2">
                <div className="flex items-center justify-between mb-6 px-2">
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="h-2 w-40 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                      <motion.div animate={{ width: `${((currentQuestion + 1) / 10) * 100}%` }} className="h-full bg-rose-500 shadow-[0_0_12px_rgba(244,63,94,0.6)]" />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-xl font-black text-rose-600 leading-none">{score}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col justify-center gap-4">
                  ...
                  >
                    ...
                    <p className="text-rose-500 font-black text-[10px] uppercase tracking-[0.3em] opacity-50">Select the meaning</p>
                  </motion.div>
                        ...
                        <motion.button
                          ...
                          className={`p-6 rounded-[1.8rem] font-black text-[13px] transition-all border-2 text-left flex items-center justify-between group relative overflow-hidden ${
                            isSelected 
                              ...
                              : 'bg-white border-slate-100 text-slate-700 active:bg-slate-50 hover:border-rose-300 shadow-sm'
                          }`}
                        >
                          ...
                          {!isSelected && <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 group-hover:bg-rose-50 group-hover:text-rose-500 transition-all shrink-0">
          ...
          <PhrasalVerbsThroughGuide />
        ) : null}
      </div>

      {/* Detail Overlay */}
      <AnimatePresence>
        {selectedIndex !== null && selectedPhrasalVerbThrough && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            <div className="flex-1 flex items-center justify-center p-6">
              <div className="flex items-center justify-center w-full max-w-3xl gap-4 h-full">
                ...
                <motion.div
                  key={selectedPhrasalVerbThrough.id}
                  initial={{ scale: 0.9, opacity: 0, y: 30 }}
                  animate={{ scale: 1, opacity: 1, y: 0 }}
                  exit={{ scale: 0.9, opacity: 0, y: -30 }}
                  className="w-full max-md bg-white rounded-[3.5rem] p-8 shadow-2xl relative flex flex-col overflow-hidden"
                ...
                >
                  ...
                  <div className="text-center flex-1 flex flex-col pt-4">
                    <div className="flex flex-col items-center gap-4 mb-8">
                      <div className="px-3 py-1 rounded-full bg-rose-50 text-rose-600 font-black text-[8px] uppercase tracking-[0.2em] shadow-inner border border-rose-100/50">
                        THROUGH MODULE • UNIT {selectedPhrasalVerbThrough.id}
                      </div>
                      <h3 className="text-5xl font-black text-slate-900 tracking-tighter leading-none">
                        {selectedPhrasalVerbThrough.word}
                      </h3>
                      <div className="bg-slate-50 px-4 py-2 rounded-2xl border border-slate-100">
                        <span className="text-rose-500 font-black text-xl italic tracking-tight">
                            {selectedPhrasalVerbThrough.pronunciation}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 mb-10 overflow-y-auto max-h-[30vh] px-2 scrollbar-hide">
                      <div className="p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100 shadow-inner text-left">
                        ...
                        <p className="text-slate-900 text-3xl font-black tracking-tighter leading-tight mb-2">{selectedPhrasalVerbThrough.spanish}</p>
                        <div className="inline-block px-3 py-1 rounded-lg bg-rose-100/50 text-rose-700 font-black text-[9px] uppercase tracking-widest">
                           {selectedPhrasalVerbThrough.category}
                        </div>
                      </div>

                      <div className="p-6 bg-rose-50/50 rounded-[2.5rem] border border-rose-100/20 relative text-left">
                        <Sparkles size={18} className="text-rose-500/20 absolute top-5 right-5" />
                        <h4 className="text-[9px] font-black text-rose-400 uppercase tracking-widest mb-2">Conceptual Meaning</h4>
                        <p className="text-slate-700 text-sm font-bold leading-relaxed">
                          "{selectedPhrasalVerbThrough.meaning}"
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mt-auto pt-6 border-t border-slate-50">
                      <button
                        onClick={() => handlePlay(selectedPhrasalVerbThrough.word, selectedPhrasalVerbThrough.id)}
                      ...
                      <div className="flex flex-col items-center gap-2 shrink-0">
                        <button
                          ...
                          className={`w-14 h-14 rounded-[1.8rem] border-2 flex items-center justify-center shadow-lg active:scale-90 transition-all ${
                            getFlagColor(flags[selectedPhrasalVerbThrough.id + 12000] || 0)
                          }`}
                        >
                          <Flag size={24} fill={(flags[selectedPhrasalVerbThrough.id + 12000] || 0) > 0 ? 'currentColor' : 'none'} strokeWidth={3} />
                        </button>
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest opacity-50">Flag Lv {(flags[selectedPhrasalVerbThrough.id + 12000] || 0)}</span>
                      </div>
...
export default PhrasalVerbsThroughSection;
