...
import { 
  ...
  setWordFlag, 
  resetWordFlag, 
  getPhrasalVerbThroughRepetitions, 
  incrementPhrasalVerbThroughRepetition, 
  clearAllPhrasalVerbThroughPersistence, 
  clearFlagsInRange 
...
const PhrasalVerbsThroughSection: React.FC<PhrasalVerbsThroughSectionProps> = ({ onBack, speakText }) => {
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        console.error("Error loading PhrasalVerbThrough stats:", err);
      }
    };
    loadStats();
  }, []);
  ...
  const categories = useMemo(() => {
    const cats = Array.from(new Set(PHRASAL_VERBS_THROUGH.map(v => v.category)));
    return ['ALL', ...cats];
  }, []);

  const progress = useMemo(() => {
    const total = PHRASAL_VERBS_THROUGH.length;
    const flagged = PHRASAL_VERBS_THROUGH.filter(v => (flags[v.id + 12000] || 0) > 0).length;
        ...
        {selectedIndex === null && (
          <>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2 pb-24">
              {activeTab === 'STUDY' && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between px-2">
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Vocabulary List</h3>
                  ...
                  {activeCategory === 'ALL' ? (
                    categories.filter(c => c !== 'ALL').map(cat => {
                      const verbsInCat = PHRASAL_VERBS_THROUGH.filter(v => v.category === cat);
                      ...
                      return (
                        <div key={cat} className="space-y-3">
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {verbsInCat.map((s) => {
                              const absoluteIdx = PHRASAL_VERBS_THROUGH.findIndex(v => v.id === s.id);
                              return (
                                <motion.button
                                  key={s.id}
                                  whileTap={{ scale: 0.98 }}
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-3">
                                    {(flags[s.id + 12000] || 0) > 0 && (
                                      <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[8px] font-black ${(flags[s.id + 12000] || 0) >= 3 ? 'bg-rose-500 text-white' : 'bg-amber-400 text-white'}`}>
                                        <Flag size={8} fill="currentColor" />
                                        <span>{(flags[s.id + 12000] || 0)}</span>
                                      </div>
                                    )}
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {filteredPhrasalVerbsThrough.map((s) => {
                        const absoluteIdx = PHRASAL_VERBS_THROUGH.findIndex(v => v.id === s.id);
                        return (
                          <motion.button
                            key={s.id}
                  ...
                  {!gameStarted ? (
                    <div className="grid grid-cols-1 gap-4 pt-4">
                      ...
                      <motion.button 
                        ...
                        className="p-8 bg-gradient-to-br from-rose-600 to-rose-900 rounded-[2.5rem] shadow-xl flex flex-col items-center text-center group text-white"
                      >
                        ...
                        <h3 className="text-lg font-black mb-2">Concept Linker</h3>
                        <p className="text-white/70 text-[10px] font-bold uppercase tracking-widest mb-6">Drag verbs to their meanings</p>
                        <div className="px-6 py-2 bg-white text-rose-600 rounded-xl font-black text-[9px] uppercase tracking-widest flex items-center gap-2">
                        ...
                        <div className="space-y-6">
                           <div className="flex items-center justify-between px-2">
                              <div className="flex flex-col gap-1">
                                 ...
                                 <div className="h-1 w-32 bg-slate-100 rounded-full overflow-hidden">
                                    <motion.div animate={{ width: `${((currentQuestion + 1) / 10) * 100}%` }} className="h-full bg-rose-500" />
                                 </div>
                              </div>
                              <div className="flex items-center gap-3">
                           ...
                           <AnimatePresence mode="wait">
                              ...
                              >
                                 ...
                                 <p className="text-rose-500 font-black text-[8px] uppercase tracking-widest opacity-60">What does it mean?</p>
                              </motion.div>
                           </AnimatePresence>
                           ...
                           <div className="flex justify-center pt-4">
                              <button onClick={() => setGameStarted(false)} className="text-[9px] font-black text-slate-300 uppercase tracking-widest hover:text-rose-500 transition-colors">Abort Mission</button>
                           </div>
                        </div>
                      )}
                    </div>
              ...
              {activeTab === 'GUIDE' && <PhrasalVerbsThroughGuide />}
            </div>
          </>
        )}
      </AnimatePresence>
            ...
            <div className="flex-1 flex items-center justify-center p-6">
               <div className="w-full max-w-sm bg-white rounded-[3rem] p-8 shadow-2xl relative overflow-hidden flex flex-col">
                  ...
                  <div className="flex items-center justify-between mb-8">
                     <div className="px-3 py-1 rounded-full bg-rose-50 text-rose-600 font-black text-[7px] uppercase tracking-widest border border-rose-100/50">
                        THROUGH MODULE
                     </div>
                     <div className="flex gap-2">
                        <button onClick={handleFlagPress} className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${getFlagColor(flags[selectedPhrasalVerbThrough.id + 12000] || 0)}`}>
                           <Flag size={14} fill={(flags[selectedPhrasalVerbThrough.id + 12000] || 0) > 0 ? "currentColor" : "none"} />
                        </button>
                  ...
                  <div className="text-center space-y-4 mb-8">
                     <h3 className="text-4xl font-black text-slate-900 tracking-tight">{selectedPhrasalVerbThrough.word}</h3>
                     <div className="inline-block px-4 py-1.5 bg-rose-50 rounded-xl text-rose-500 font-black text-lg italic">
                  ...
                  <div className="space-y-4 flex-1">
                     <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
                        ...
                        <p className="text-xl font-black text-slate-800">{selectedPhrasalVerbThrough.spanish}</p>
                     </div>
                     <div className="p-5 bg-rose-50/50 rounded-2xl border border-rose-100/50">
                        <p className="text-[8px] font-black text-rose-500 uppercase tracking-widest mb-1">Conceptual Meaning</p>
                        <p className="text-xs font-bold text-slate-600 leading-relaxed italic">"{selectedPhrasalVerbThrough.meaning}"</p>
                     </div>
                  </div>

                  <div className="mt-8 pt-6 border-t border-slate-50 flex gap-3">
                     <button onClick={goToPrevious} className="w-12 h-12 rounded-2xl bg-slate-100 text-slate-400 flex items-center justify-center active:scale-90 transition-all">
                        <ChevronLeft size={20} />
                     </button>
                     <button 
                     ...
                     <button onClick={goToNext} className="w-12 h-12 rounded-2xl bg-slate-100 text-slate-400 flex items-center justify-center active:scale-90 transition-all">
